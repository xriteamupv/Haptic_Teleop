...
class HandTracker:
    ...
    def get_keypoint_classifier(self, landmark_list):
        return self.tracker.get_keypoint_classifier(landmark_list)
    
    def get_point_history_classifier(self):
        return self.tracker.get_point_history_classifier(self.history.pre_processed_point_history_list)
    ...
    def configure_detection(self, gloves_color):
        self.content.image = self.tracker.configure_detection(self.content.image, gloves_color)
    ...
    def pre_process_landmark(self, landmark_list):
        return self.history.pre_process_landmark(landmark_list)

    def pre_process_point_history(self):
        image_width, image_height = self.get_display_image_properties()
        return self.history.pre_process_point_history([image_width, image_height])
    ...
    def is_grip_happening(self):
        return self.tracker.is_grip_happening()
    
    def detect_grip_level(self):
        return self.tracker.detect_grip_level()
    
    def detect_grip_fingers(self):
        return self.tracker.detect_grip_fingers()
    ...
    def hand_sign_classification(self):
        ...
        if self.state.is_robot_running() and self.is_grip_happening():
            self.grip_level = self.detect_grip_level()
            self.grip_fingers = self.detect_grip_fingers()
            self.grip_detected = True
        else:
            self.grip_level = 0
            self.grip_detected = False
        if self.get_hand_sign_id() == 2 and bool_near_reference and self.state.is_sufficient_count():
