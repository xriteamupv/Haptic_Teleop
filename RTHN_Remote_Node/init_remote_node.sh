...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) 
            ...
            #bash gnome-terminal --tab -- python3 C:/Users/rduser/Documents/FernandoHernandez/haptic_ur5e/src/haptic_control.py
            #start python haptic_control.py "--max_fingers" "$max_fingers" "--min_intensity" "$min_intensity" "--max_intensity" "$max_intensity" \
            #    "--precision_duration" "$precision_duration" "--initial_delay" "$initial_delay" "--final_delay" "$final_delay" "--intensity_shift" "$intensity_shift" \
            #    "--static_intensity" "$static_intensity" "--intensity_model" "$intensity_model" "--max_width" "$max_width" "--max_force" "$max_force" \
            #    "--right_hand_enabled" "$right_hand_enabled" "--discrete_sensations" "$discrete_sensations" "--bidirectional" "$bidirectional_haptic"
            #sleep 2
            if [[ "$use_static_image_mode" == "True" ]]; then
                python3 /mnt/c/Users/rduser/Documents/FernandoHernandez/haptic_ur5e/tracking_control.py "--device" "$device" \
                "--width" "$width" "--height" "$height" "--initial_position" "$initial" "--tolerance" "$tolerance" "--tracking" "$tracking" \
                "--gloves_color" "$color" "--use_static_image_mode" "--min_detection_confidence" "$min_detection_confidence" \
                "--record" "$record" "--with_orientation" "$with_orientation" "--bidirectional" "$bidirectional_tracking"
            else
                python3 /mnt/c/Users/rduser/Documents/FernandoHernandez/haptic_ur5e/tracking_control.py "--device" "$device" \
                "--width" "$width" "--height" "$height" "--initial_position" "$initial" "--tolerance" "$tolerance" "--tracking" "$tracking" \
                "--gloves_color" "$color" "--min_detection_confidence" "$min_detection_confidence" \
                "--record" "$record" "--with_orientation" "$with_orientation" "--bidirectional" "$bidirectional_tracking"
            fi
            break;;
