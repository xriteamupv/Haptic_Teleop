...
class RobotArm:
    ...
    def __init__(self, args):
        ...
        self.configure_start_poses(args.initial)
        self.inverted = args.inverted # Only Z axis
        self.mimic = args.mimic # X and Y axis
    ...
    def limit_movement(self, pose_element):
        if pose_element[0] < self.min_limits[0]:
            pose_element[0] = self.min_limits[0]
        if pose_element[0] > self.max_limits[0]:
            pose_element[0] = self.max_limits[0]

        if pose_element[2] > self.min_limits[2]:
            pose_element[2] = self.min_limits[2]
        if pose_element[2] < self.max_limits[2]:
            pose_element[2] = self.max_limits[2]

        if pose_element[1] > self.min_limits[1]:
            pose_element[1] = self.min_limits[1]
        if pose_element[1] < self.max_limits[1]:
            pose_element[1] = self.max_limits[1]
        return pose_element
