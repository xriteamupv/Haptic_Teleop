...
class TrajectoryClient:
    ...
    def configure_oneaxis(self, pose):
        geometry_pose = geometry_msgs.Pose(
                    ...
                    geometry_msgs.Quaternion(self.robot_arm.orientation[0], self.robot_arm.orientation[1], self.robot_arm.orientation[2], self.robot_arm.orientation[3]))
        if self.oneaxis == 0:
            print("POSE: ", pose)
    ...
    def configure_pose_arm(self):
        if self.mapping == 1:
            pose_element = TrajectoryMapper.focused_linear_mapping_arm(self.human_arm, self.robot_arm)
        elif self.mapping == 2:
            pose_element = TrajectoryMapper.multi_focused_linear_mapping_arm(self.human_arm, self.robot_arm)
        elif self.mapping == 3:
            pose_element = TrajectoryMapper.non_linear_mapping_arm(self.human_arm, self.robot_arm)
        else:
            pose_element = TrajectoryMapper.linear_mapping_arm(self.human_arm, self.robot_arm) #default
