...
{
    if [[ "$inverted" == "0" ]]; then
        echo "INVERTED: Disabled (default: False)"
    else
        inverted="1"
        echo "INVERTED: Enabled (True)"
    fi
}

BidirectionalOptions()
{
    if [[ "$bidirectional_comms" == "0" ]]; then
        echo "BIDIRECTIONAL: Disabled (default: False)"
    else
        bidirectional_comms="1"
        echo "BIDIRECTIONAL: Enabled (True)"
    fi
}
...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) 
            ...
            gnome-terminal --tab -- rosrun haptic_ur5e robot_control.py "--communication_mode" "$communication_mode" "--mapping" "$mapping" "--controller" "$controller" \
            "--initial_pos" "$initial" "--oneaxis" "$oneaxis" "--twoaxis" "$twoaxis" "--duration" "$duration" "--velocity" "$velocity" "--acceleration" "$acceleration" \
            "--precision" "$precision" "--wait_time" "$wait_time" "--orientation" "$orientation" "--mimic" "$mimic" "--inverted" "$inverted" "--bidirectional" "$bidirectional_comms"
            sleep 2
            gnome-terminal --tab -- rosrun haptic_ur5e gripper_control.py "--communication_mode" "$communication_mode" "--max_width" $max_width "--max_force" $max_force \
            "--start_width" $start_width "--width_tolerance" $width_tolerance "--force_tolerance" $force_tolerance "--max_duration" $max_duration "--time_max_width" $time_max_width \
