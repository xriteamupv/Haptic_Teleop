...
class TrajectoryMapper:
    ...
    def linear_mapping_arm(human_arm, robot_arm):
        ...
        # EJE HUMAN X, ROBOT X
        proportion_element = (human_arm.max_limits[0]-human_arm.min_limits[0])*(robot_arm.limitator.max_limits[0]-robot_arm.limitator.min_limits[0])
        pose_element[0] = robot_arm.limitator.min_limits[0] + proportion_element*human_arm.pose_list[0]

        # EJE HUMAN Y, ROBOT Z
        proportion_element = (human_arm.max_limits[1]-human_arm.min_limits[1])*(robot_arm.limitator.max_limits[2]-robot_arm.limitator.min_limits[2])
        pose_element[2] = robot_arm.limitator.min_limits[2] + proportion_element*human_arm.pose_list[1]

        # EJE HUMAN Z, ROBOT Y
        proportion_element = 500*(human_arm.max_limits[2]-human_arm.min_limits[2])*(robot_arm.limitator.max_limits[1]-robot_arm.limitator.min_limits[1])
        pose_element[1] = robot_arm.limitator.min_limits[1] + proportion_element*human_arm.pose_list[2]

        pose_element = robot_arm.limit_movement(pose_element)
        pose_element[0] = robot_arm.limitator.max_limits[0] - (pose_element[0] - robot_arm.limitator.min_limits[0])
        if robot_arm.mimic:
            print("MIMIC ON")
            pose_element[0] = robot_arm.limitator.max_limits[0] - (pose_element[0] - robot_arm.limitator.min_limits[0])
            pose_element[1] = robot_arm.limitator.max_limits[1] - (pose_element[1] - robot_arm.limitator.min_limits[1])
        if robot_arm.inverted:
            print("INVERTED ON")
            pose_element[2] = robot_arm.limitator.max_limits[2] - (pose_element[2] - robot_arm.limitator.min_limits[2])
    ...
    def focused_linear_mapping_arm(human_arm, robot_arm):
        ...
        for coord in range(len(["x","y","z"])):
            ...
            conditioner = TrajectoryConditioner(coord, robot_arm.mimic, robot_arm.inverted, robot_arm.limitator.max_limits[coord]-robot_arm.limitator.min_limits[coord])
            #if human_arm.max_limits[coord] == -human_arm.min_limits[coord]:
            #    slope_diff = [-0.2, -0.2, -0.05]
            #else:
            #    slope_diff = [0.08, 0.08, -0.05]
            
            proportion_m1 = (robot_arm.limitator.max_limits[coord] - robot_arm.limitator.min_limits[coord]) / (human_arm.max_limits[coord] - human_arm.min_limits[coord])
            proportion_m2 = proportion_m1 + conditioner.slope_diff_focused
            
            if human_arm.pose_list[coord] < min_foc_human_arm:
                pose_element[coord] = TrajectoryMapper.calc_slope(robot_arm.limitator.min_limits[coord], proportion_m1, human_arm.pose_list[coord], human_arm.min_limits[coord])
                ...
                pose_element[coord] = TrajectoryMapper.calc_slope(robot_arm.limitator.min_limits[coord], proportion_m1, x2, human_arm.min_limits[coord])
            else:
                xm2 = TrajectoryMapper.calc_slope(min_foc_human_arm, proportion_m2/proportion_m1, max_foc_human_arm, min_foc_human_arm)
                proportion_m3 = (robot_arm.limitator.max_limits[coord] - TrajectoryMapper.calc_slope(robot_arm.limitator.min_limits[coord], proportion_m1, xm2,human_arm.min_limits[coord]))/(human_arm.max_limits[coord] - max_foc_human_arm)
            ...
            pose_element[coord] = max(min(robot_arm.limitator.min_limits[coord], robot_arm.limitator.max_limits[coord]), min(max(robot_arm.limitator.min_limits[coord], robot_arm.limitator.max_limits[coord]), pose_element[coord]))
    ...
    def multi_focused_linear_mapping_arm(human_arm, robot_arm):
        ...
        for coord in range(len(["x","y","z"])):
            ...
            proportion_m1 = (human_arm.max_limits[coord]-human_arm.min_limits[coord])*(robot_arm.limitator.max_limits[coord]-robot_arm.limitator.min_limits[coord]) + conditioner.slope_diffs_multifocused[0] # m1
            proportion_m2 = (human_arm.max_limits[coord]-human_arm.min_limits[coord])*(robot_arm.limitator.max_limits[coord]-robot_arm.limitator.min_limits[coord]) + conditioner.slope_diffs_multifocused[1] # m2
            proportion_m3 = (human_arm.max_limits[coord]-human_arm.min_limits[coord])*(robot_arm.limitator.max_limits[coord]-robot_arm.limitator.min_limits[coord]) + conditioner.slope_diffs_multifocused[2] # m3
            proportion_m4 = (human_arm.max_limits[coord]-human_arm.min_limits[coord])*(robot_arm.limitator.max_limits[coord]-robot_arm.limitator.min_limits[coord]) + conditioner.slope_diffs_multifocused[3] # m4
            
            if human_arm.pose_list[coord] < min_foc_human_arm:
                pose_element[coord] = TrajectoryMapper.calc_slope(robot_arm.limitator.min_limits[coord], proportion_m1, human_arm.pose_list[coord], 0)
            elif min_foc_human_arm <= human_arm.pose_list[coord] < midmin_foc_human_arm:
                x2 = TrajectoryMapper.calc_slope(min_foc_human_arm, proportion_m2/proportion_m1, human_arm.pose_list[coord], min_foc_human_arm)
                pose_element[coord] = TrajectoryMapper.calc_slope(robot_arm.limitator.min_limits[coord], proportion_m1, x2, 0)
                ...
                proportion_m5 = (robot_arm.limitator.max_limits[coord] - TrajectoryMapper.calc_slope(robot_arm.limitator.min_limits[coord], proportion_m1, xm2,0))/(human_arm.max_limits[coord] - max_foc_human_arm)
    ...
    def non_linear_mapping_arm(human_arm, robot_arm):
        ...
        for coord in range(len(["x","y","z"])):
            ...
            proportion_m1 = (human_arm.max_limits[coord]-human_arm.min_limits[coord])*(robot_arm.limitator.max_limits[coord]-robot_arm.limitator.min_limits[coord]) + conditioner.slope_diff_nonlinear # m1
        
            if human_arm.pose_list[coord] < min_foc_human_arm:
                pose_element[coord] = TrajectoryMapper.calc_slope(robot_arm.limitator.min_limits[coord], proportion_m1, human_arm.pose_list[coord], 0)
            elif min_foc_human_arm <= human_arm.pose_list[coord] <= max_foc_human_arm:
                y2 = TrajectoryMapper.calc_slope(robot_arm.limitator.min_limits[coord], proportion_m1, min_foc_human_arm, human_arm.min_limits[coord])
                ...
                proportion_m2 = (robot_arm.limitator.max_limits[coord] - y3)/(human_arm.max_limits[coord] - max_foc_human_arm)
                pose_element[coord] = TrajectoryMapper.calc_slope(y3, proportion_m2, human_arm.pose_list[coord], max_foc_human_arm)

            if robot_arm.limitator.min_limits[coord] < robot_arm.limitator.max_limits[coord]:
                pose_element[coord] = max(robot_arm.limitator.min_limits[coord], min(robot_arm.limitator.max_limits[coord], pose_element[coord]))
            else:
                pose_element[coord] = max(robot_arm.limitator.max_limits[coord], min(robot_arm.limitator.min_limits[coord], pose_element[coord]))
